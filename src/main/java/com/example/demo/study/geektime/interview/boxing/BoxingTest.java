 ...
 *     </pre>
 *     对象实例(Instance Data)：对象真正存储的有效信息，程序中定义的各种类型的字段内容，包括从父类继承的和子类自身定义的
 *     对齐填充(Padding)：非必然存在，起占位符的作用，因为 HotSpot JVM 的自动内存管理要求对象起始地址必须是8字节的整数倍，即对象大小必须是8字节的整数倍
 * </pre>
 *
 * @author chris
 */
